...
router.get('/:id', catchAsync(async (req, res) => {
    const { id } = req.params
    const league = await League.findById(id).select('_id name country tier value teams photo')
...
router.patch('/:id', catchAsync(async (req, res) => {
    ...
    const { id } = req.params;
    const { name, country, tier, photo, url } = req.body

    let league = await League.findById(id)

    // res.render('editTeam', { echipa, leagues });
    league = Object.assign(league, { name, country, tier, photo, url })
    await league.save()
